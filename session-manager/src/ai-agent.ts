...
11. **שיחה חדשה** — ברכה קצרה בלבד (בוקר/צהריים/ערב טוב + שם העסק + "איך אפשר לעזור?"). **אל תניח הנחות** על מה הלקוח צריך — אל תדבר על מחירים, אמינות, מבצעים או שירותים עד שהלקוח שואל.
...
export async function generateReply(
    ...
    if (history.length <= 1) {
        systemPrompt += `\n\n[שיחה חדשה — ברכה קצרה בלבד: "שלום! אני העוזר הווירטואלי של <שם העסק>. איך אפשר לעזור?" אל תניח הנחות על מה הלקוח רוצה, אל תדבר על אמינות/מחירים/מבצעים עד שהלקוח שואל. רק ברכה + שאלה איך לעזור.]`;
    }
