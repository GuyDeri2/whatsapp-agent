...
async function buildSystemPrompt(tenantId: string): Promise<string> {
    ...
    if (kbEntries && kbEntries.length > 0) {
        const kbText = kbEntries
            .map((e: { question: string; answer: string; category: string | null }) => `ש: ${e.question}\nת: ${e.answer}${e.category ? ` [${e.category}]` : ""}`)
            .join("\n\n");
        prompt += `\n\n<knowledge_base>\n${kbText}\n</knowledge_base>`;
    }
...
2. **נושאי העסק בלבד** — ענה רק על סמך המידע שסופק לך (הגדרות העסק, בסיס הידע, הוראות בעל העסק). **אסור להשתמש בידע כללי שלך.** אם המידע לא מופיע במקורות שניתנו לך — קודם כל, שאל את הלקוח מה בדיוק הוא מחפש (אולי השאלה מתייחסת למשהו שכן קיים במאגר). אם אחרי שאלת הבהרה עדיין אין לך מידע רלוונטי — אמור שאתה לא בטוח והצע להעביר לנציג.
...
8. **העברה לנציג** — העבר לנציג (סיים ב-[PAUSE]) אם: הלקוח מבקש נציג, מביע כעס/תסכול, תלונה מורכבת, מידע חסר אחרי ניסיון לעזור, 2 ניסיונות הבהרה נכשלו, בעיות תשלום/טכניות, או **שאלות על אלרגנים/אלרגיות** (ראה כלל 19).
...
export async function generateReply(
    ...
    if (history.length <= 1) {
        ...
        systemPrompt += `\n\n[שיחה חדשה — תגובה קצרה בפורמט הזה בלבד: "${greeting}, אני העוזר הווירטואלי של <שם העסק>. במה אוכל לעזור היום?" ותו לא. אל תוסיף שום מידע נוסף.]`;
    }
