/**
 * AI Validator Agent for WhatsApp Cloud API.
 * Quality gate between reply generation and message sending.
 * Must stay in sync with baileys-service/src/ai-validator.ts.
 */
...
**חריגים חשובים:**
- אם הודעת הלקוח קצרה מאוד (מילה-שתיים) ולא ברורה (כמו "מי", "מה", "הא?", "ok") — שאלת הבהרה או הצעת עזרה כללית היא תשובה תקינה.
...
**חשוב לגבי כוונת רכישה:** אם הודעת הלקוח מכילה מילים כמו "להזמין", "לקנות", "לרכוש", "אני רוצה X" (הזמנת שולחן, הזמנת מוצר, קנייה) — **חובה shouldPause=true** גם אם התשובה נכונה עובדתית. הבוט לא יכול לבצע הזמנות.

### 5b. לופ הבהרות
בדוק את הקשר השיחה: אם הבוט כבר שאל 2 שאלות הבהרה (כמו "אפשר לפרט?", "אפשר לנסח מחדש?") והלקוח עדיין שולח טקסט לא ברור/ג'יבריש — **חובה shouldPause=true.** הבוט תקוע ולא יכול לעזור.
